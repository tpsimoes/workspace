...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'sudo su - '${sapid}'adm -c "python /usr/sap/'${sapID}'/HDB'${instanceID}'/exe/python_support/systemReplicationStatus.py"'  --query "value[].message" -o tsv;
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'sudo su - '${sapid}'adm -c "python /usr/sap/'${sapID}'/HDB'${instanceID}'/exe/python_support/landscapeHostConfiguration.py"'  --query "value[].message" -o tsv;
