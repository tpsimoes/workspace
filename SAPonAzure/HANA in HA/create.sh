...
################# need to upload SAP file 51056431.ZIP to the container

# On this section need to be improved to somehow - wait for user to upload the file to the container!!!

################################################################

################# Download & Install AZCopy Tool & Copy SAP File

###### Create Directories
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'mkdir -p /hana/data/'${sapID}'/mnt00001 && mkdir -p /hana/data/'${sapID}'/mnt00002 && mkdir -p /hana/log/'${sapID}'/mnt00001 &&  mkdir -p /hana/log/'${sapID}'/mnt00002 && mkdir -p /hana/shared && mkdir -p /usr/sap/'${sapID}''
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname2 --command-id RunShellScript --scripts 'mkdir -p /hana/data/'${sapID}'/mnt00001 && mkdir -p /hana/data/'${sapID}'/mnt00002 && mkdir -p /hana/log/'${sapID}'/mnt00001 &&  mkdir -p /hana/log/'${sapID}'/mnt00002 && mkdir -p /hana/shared && mkdir -p /usr/sap/'${sapID}''
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname3 --command-id RunShellScript --scripts 'mkdir -p /hana/data/'${sapID}'/mnt00001 && mkdir -p /hana/data/'${sapID}'/mnt00002 && mkdir -p /hana/log/'${sapID}'/mnt00001 &&  mkdir -p /hana/log/'${sapID}'/mnt00002 && mkdir -p /hana/shared && mkdir -p /usr/sap/'${sapID}''
...
###### For now its downloading from my personal StorageAccount
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts "cd /usr/sap/'${sapID}'/azcopy_linux_amd64_10.17.0 && ./azcopy copy 'https://sapsoftwarerepro.blob.core.windows.net/sapsoftware?sp=rl&st=2023-03-22T15:18:28Z&se=2024-04-01T22:18:28Z&sv=2021-12-02&sr=c&sig=9qKiJLTdADcaG8XzgiedOLyp6Wr9yU6RqUn59f03KiU%3D' '/hana/shared/'${sapID}'/download' --recursive=TRUE"
...
# [A] Update Hosts 
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'echo -e "# Storage\n'${hanadb1_storage_pip}'   hanadb1-storage\n'${hanadb2_storage_pip}'   hanadb2-storage\n'${hanadb3_storage_pip}'   hanadb3-storage\n# Client\n'${hanadb1_client_pip}'   hanadb1\n'${hanadb2_client_pip}'   hanadb2\n'${hanadb3_client_pip}'   hanadb3\n# Hana\n'${hanadb1_hana_pip}'  hanadb1-hana\n'${hanadb2_hana_pip}'    hanadb2-hana\n'${hanadb2_hana_pip}'    hanadb3-hana\n" >> /etc/hosts';
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'mkdir /mnt/tmp && mount -t nfs -o sec=sys,vers=4.1 '${netappIP}':/'${sapID}'-shared /mnt/tmp && cd /mnt/tmp && mkdir shared usr-sap-'${vmsapname1}' usr-sap-'${vmsapname2}' usr-sap-'${vmsapname3}' && cd.. && umount /mnt/tmp'

# [2,3] Create node-specific directories for /usr/sap on HN1-shared
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname2 --command-id RunShellScript --scripts 'mkdir /mnt/tmp && mount -t nfs -o sec=sys,vers=4.1 '${netappIP}':/'${sapID}'-shared /mnt/tmp && umount /mnt/tmp'
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'groupadd -g 1001 sapsys && useradd '${sapid}'adm -u 1001 -g 1001 -d /usr/sap/'${sapID}'/home -c "SAP HANA Database System" -s /bin/sh && useradd sapadm -u 1002 -g 1001 -d /home/sapadm -c "SAP Local Administrator" -s /bin/sh && (echo 'Password1!'; echo 'Password1!') | passwd '${sapid}'adm && (echo 'Password1!'; echo 'Password1!') | passwd sapadm'
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname2 --command-id RunShellScript --scripts 'groupadd -g 1001 sapsys && useradd '${sapid}'adm -u 1001 -g 1001 -d /usr/sap/'${sapID}'/home -c "SAP HANA Database System" -s /bin/sh && useradd sapadm -u 1002 -g 1001 -d /home/sapadm -c "SAP Local Administrator" -s /bin/sh && (echo 'Password1!'; echo 'Password1!') | passwd '${sapid}'adm && (echo 'Password1!'; echo 'Password1!') | passwd sapadm'

# [A] Mount the shared Azure NetApp Files volumes
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'echo -e "# Add the following entries\n'${netappIP}':/'${sapID}'-data-mnt00001 /hana/data/'${sapID}'/mnt00001  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0\n'${netappIP}':/HN1-data-mnt00002 /hana/data/HN1/mnt00002  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0\n'${netappIP}':/HN1-log-mnt00001 /hana/log/HN1/mnt00001  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0\n'${netappIP}':/HN1-log-mnt00002 /hana/log/HN1/mnt00002  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0\n'${netappIP}':/HN1-shared /hana/shared  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0" >> /etc/fstab && mount -a'
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'echo -e "# Add the following entries\n'${netappIP}':/'${volume_usrsap1_path}' /usr/sap/'${sapID}'  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0" >> /etc/fstab && mount -a'

# [2] Mount the node-specific volumes on hanadb2

az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname2 --command-id RunShellScript --scripts 'echo -e "# Add the following entries\n'${netappIP}':/'${volume_usrsap2_path}'  /usr/sap/'${sapID}'  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0" >> /etc/fstab && mount -a'

# [3] Mount the node-specific volumes on hanadb3 - POR CORRER!!

az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname3 --command-id RunShellScript --scripts 'echo -e "# Add the following entries\n'${netappIP}':/'${volume_usrsap3_path}'  /usr/sap/'${sapID}'  nfs   rw,vers=4,minorversion=1,hard,timeo=600,rsize=262144,wsize=262144,intr,noatime,lock,_netdev,sec=sys  0  0" >> /etc/fstab && mount -a'
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'sudo zypper install libgcc_s1 libstdc++6 libatomic1 insserv-compat libtool';
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname2 --command-id RunShellScript --scripts 'sudo zypper install libgcc_s1 libstdc++6 libatomic1 insserv-compat libtool';
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'unzip -d /hana/shared/'${sapID}'/download/hanainstall /hana/shared/'${sapID}'/download/sapsoftware/51056431.ZIP';
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'cat ~/hdb_passwords.xml | ./hdblcm --batch --action=install --components=client,server --sid='${sapID}' --number='${instanceID}' --read_password_from_stdin=xml'
...
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname2 --command-id RunShellScript --scripts 'cat ~/hdb_passwords.xml | ./hdblcm --batch --action=add_hosts --addhosts='${vmsapname2}':role='${vmsapname2_role}' --read_password_from_stdin=xml'

# [A]

# Confirm HA - Two Nodes
az vm run-command invoke -g $RESOURCE_GROUP -n $vmsapname1 --command-id RunShellScript --scripts 'su - '${sapid}'adm && python /usr/sap/'${sapID}'/HDB'${instanceID}'/exe/python_support/landscapeHostConfiguration.py' 
