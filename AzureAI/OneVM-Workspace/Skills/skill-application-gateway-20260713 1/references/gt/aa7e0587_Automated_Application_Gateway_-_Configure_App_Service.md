...
> **Solution ID:** aa7e0587-d25c-4802-b60e-808265fb7888  
...
This guide provides step-by-step troubleshooting for **Automated] Application Gateway - Configure App Service** under **Application Gateway**.
 The original guided troubleshooter contains 23 steps covering scoping, diagnosis, and resolution.
...
3. If Application Gateway is using Service Endpoint on the subnet and Access from all networks is disabled on the App Service, ensure the Application Gateway virtual network is allowed in App Service access restrictions.

- Ask cx to open the App Service in Azure Portal

- Select Networking tab

- Open Access Restrictions

- Verify the Application Gateway VNet/Subnet is added as an Allow rule.If not present, add a Virtual Network–based access restriction for the App Gateway subnet
...
3. If Application Gateway is using Service Endpoint on the subnet and Access from all networks is disabled on the App Service, ensure the Applica
...
- **Issues with adding custom domain in App Service** → Go to: *Domain Validation*
...
You can confirm this by checking either the server name within [BackendServerDiagnosticHistory](https://portal.microsoftgeneva.com/s/5EE80A21) table in Jarvis logs, under AppGWT namespace or

 by checking jarvis action query [Get List of NonResolvable Domains](https://portal.microsoftgeneva.com/8ABEA554?genevatraceguid=fbc20328-33c1-44e4-a9ba-17aa1550837b)

### Question

**Is the configured custom domain resolving to correct IP.**

### Options

- **yes** → Go to: *Custom domain 502 backend healthy*
- **no** → Go to: *Custom domain 502 private endpoint*
...
If you are having issues with adding Custom domain in the App service follow the below document 

[Troubleshoot Domain validation issues in App service](https://learn.microsoft.com/en-us/troubleshoot/azure/app-service/connection-issues-with-ssl-or-tls/troubleshoot-domain-and-tls-ssl-certificates)
...
### Step 20: Custom domain 502 backend healthy

### Guidance

Check Backend Connectivity Diagnostic (For v2 SKU) under Diagnostics section of ASC for the given Application Gateway. 
...
- **Yes** → Go to: *d88f3f43-0cc3-4143-aced-c4820b56e8ec*
- **No** → Go to: *Custom domain 502 probe live traffic difference*

---

### Step 21: Custom domain 502 probe live traffic difference

### Support Engineer Solution

If the backend is healthy but live traffic fails with 502, ensure that the hostname configured in the probe matches the hostname used for live/backend traffic.

- Confirm the probe hostname matches the backend/live traffic hostname

- Go to Jarvis Logs > AppGWT > BackendServerDiagnosticHistory

- Check the probe hostname under the ServerName column

- Verify the hostname configured in backend settings

- If no hostname is specified in backend settings, confirm the incoming user request hostname, as that is what will be sent to the App Service

						

If the intended hostname is not configured in the probes but is on live traffic, correct the same in probe setting and the backend health should show correct live health.
...
### Step 22: Custom domain 502 private endpoint
...
**Is the custom domain configured with private endpoint**

### Options

- **Yes** → Go to: *Private Endpoint Configuration*
- **No** → Go to: *custom domain 502 DNS resolution*

---

### Step 23: custom domain 502 DNS resolution
...
If the Application Gateway virtual network is configured with custom DNS servers, ensure that the Application Gateway can successfully communicate with those DNS server(s).

To validate resolution, perform an nslookup for the backend FQDN from a VM in the same virtual network as the Application Gateway. This helps confirm whether the custom DNS server is able to resolve the backend FQDN correctly.

If DNS servers were recently modified on the virtual network, ensure the Application Gateway has been Stopped and Started for the DNS changes to take effect.
...
If the backend FQDN is a public domain, attempt to resolve it from your local machine or from a VM in the same virtual network as the Application Gateway zone.
...
DNS resolution troubleshooting steps: https://aka.ms/dnsresolutionerror

Backend health and resolution issues: https://aka.ms/UnknownBackendHealth
