...
> **Solution ID:** 9b713eb1-00e9-4d58-8180-71f9ce4768bf  
> **Trigger words:** application, application gateway, backend, connectivity, connectivity issue, gateway

---

## Overview

This guide provides step-by-step troubleshooting for **Application Gateway - Backend Connectivity** under **Application Gateway**.
...
3.	Check whether the virtual network is configured with a custom DNS server. If it is, check the DNS server to see why it can't resolve to the IP address of the specified FQDN.
...
5.	If the domain is private or internal, try to resolve it from a VM in the same virtual network. If you can resolve it, restart Application Gateway and check again. To restart Application Gateway, you need to [stop](https://learn.microsoft.com/en-us/powershell/module/azurerm.network/stop-azurermapplicationgateway) and [start](https://learn.mic
...
For more details on how to verify the above, see [visit this documentation.](https://learn.microsoft.com/en-us/azure/application-gateway/application-gateway-backend-health-troubleshooting#backend-health-status-unknown)
...
**Which of the following messages is displayed in the ‘Details’ column under Backend Heath page?**

### Options

- **Backend server timeout - Time taken by the backend to respond to application gateways health probe is more than the timeout threshold in the probe setting.** → Go to: *Backend server timeout*
- **DNS resolution error - Application Gateway could not create a probe for this backend. This usually happens when the FQDN of the backend has not been entered correctly.** → Go to: *DNS Resolution Error*
- **Updates to the DNS entries of the backend pool - The backend health status could not be retrieved.** → Go to: *Updates to the DNS entries of the backend pool*
- **TCP connect error - Application Gateway could not connect to the backend. Check that the backend responds on the port used for the probe. Also check whether any NSG/UDR/Firewall is blocking access to the Ip and port of this backend.** → Go to: *TCP Connect Error*
- **HTTP status code mismatch - Status code of the backend's HTTP response did not match the probe setting. Expected:{HTTPStatusCode0} Received:{HTTPStatusCode1}.** → Go to: *HTTP Status code mismatch*
- **HTTP response body mismatch - Body of the backend's HTTP response did not match the probe setting. Received response body doesn't contain {string}.** → Go to: *HTTP response body mismatch*
- **Common Name (CN) doesn't match - (For V2) The Common Name of the leaf certificate presented by the backend server does not match the Probe or Backend Setting hostname of the application gateway. (For V1) The Common Name (CN) of the backend certificate doesn’t match.** → Go to: *Common Name does not match*
- **Backend certificate has expired - Backend certificate is invalid. Current date is not within the "Valid from" and "Valid to" date range on the certificate.** → Go to: *Backend certificate has expired*
- **The intermediate certificate was not found - The Intermediate certificate is missing from the certificate chain presented by the backend server. Ensure the certificate chain is complete and correctly ordered on the backend server.** → Go to: *The intermediate certificate was not found*
- **The leaf or server certificate was not found - The Leaf certificate is missing from the certificate chain presented by the backend server. Ensure the chain is complete and correctly ordered on the backend server.** → Go to: *The leaf or server certificate was not found*
- **Server certificate is not issued by a publicly known CA - The backend Server certificate is not signed by a well-known Certificate Authority (CA). To use unknown CA certificates, its Root certificate must be uploaded to the Backend Setting of the application gateway.** → Go to: *Server certificate is not issued by a publicl
